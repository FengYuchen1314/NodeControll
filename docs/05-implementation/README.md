...
- [WP-02-C4 TOTP 核心实现](./WP02_C4_TOTP_IMPLEMENTATION.md)：记录固定 TOTP profile、零化 seed/code、受控管理身份绑定、active/pending 共存、原子 activation/disable、恢复码与认证版本联动，以及 C3 reservation-time/replay CAS 接线；HTTP/OpenAPI/Vue/base32/otpauth 与 credential-aware rewrap 尚未实现。
- [WP-02-C5 WebAuthn 核心实现](./WP02_C5_WEBAUTHN_IMPLEMENTATION.md)：精确 public origin/RP、`webauthn-rs` 0.5.5 typed/encrypted single-use ceremony、多 credential lifecycle、C3 crash-resume claim、BE/BS-aware counter CAS、clone-suspected 与双数据库合同。当前只包含 domain/secrets/application/persistence；HTTP/OpenAPI/Vue 和提交级 VPS 证据尚缺。
