...
- [WP-02/P5.3 密码登录与服务端会话纵切](./WP02_AUTH_SESSION_SLICE.md)：共享登录限流、全路径并发闸门、HMAC 会话、idle/absolute 期限、Origin/Host/CSRF、可信代理、登录/恢复/退出前端状态机；当前候选树预检全绿，仍待同一公开 SHA 的 Actions + fresh-checkout 正式门。MFA、token、完整 RBAC 与用户生命周期尚未进入本纵切。
