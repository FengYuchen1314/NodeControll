...
- 许可证收集器的断网双跑属于历史发布前诊断；正式 notices/SBOM 只由 Actions 生成。VPS 对 artifact 内 inventory、许可证正文/checksum、CycloneDX schema 和 override 闭包做只读验证，并把 fresh `.pnpm` 的实际 npm identity 与 inventory 双向比对，不再生成第二份正式 payload。当前 C1 仍须等待同 SHA Actions artifact 才能登记正式 provenance。
