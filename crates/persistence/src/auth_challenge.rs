...
impl Database {
    ...
    /// inside its lease; a method-specific terminal transaction may instead pin the exact TOTP or
    /// WebAuthn handoff until the enclosing challenge expires.
...
async fn refresh_access_sqlite(
    ...
    .await?;
    sqlx::query(
        "UPDATE auth_challenges SET status=CASE WHEN attempts_used=max_attempts THEN 'exhausted' ELSE 'pending' END,attempt_claim_id=NULL,attempted_method=NULL,attempt_started_at_ms=NULL,attempt_expires_at_ms=NULL,revision=revision+1,updated_at_ms=? WHERE id=? AND token_key_version=? AND token_hmac=? AND context_key_version IS ? AND client_network_hmac IS ? AND user_agent_hash IS ? AND status='verification_pending' AND created_at_ms<=? AND expires_at_ms>? AND attempt_expires_at_ms<=? AND NOT ((attempted_method='totp' AND EXISTS(SELECT 1 FROM totp_verified_handoffs h WHERE h.challenge_id=auth_challenges.id AND h.attempt_claim_id=auth_challenges.attempt_claim_id AND h.challenge_revision=auth_challenges.revision AND h.reserved_at_ms=auth_challenges.attempt_started_at_ms AND h.verification_expires_at_ms=auth_challenges.attempt_expires_at_ms)) OR (attempted_method='webauthn' AND EXISTS(SELECT 1 FROM webauthn_ceremonies AS wc WHERE wc.kind='authentication' AND wc.status IN ('consumed','rejected') AND wc.auth_challenge_id=auth_challenges.id AND wc.claim_id=auth_challenges.attempt_claim_id AND wc.user_id=auth_challenges.user_id AND wc.session_id IS auth_challenges.session_id AND wc.purpose=auth_challenges.purpose AND wc.auth_revision=auth_challenges.auth_revision AND wc.reserved_at_ms=auth_challenges.attempt_started_at_ms AND wc.verification_expires_at_ms=auth_challenges.attempt_expires_at_ms AND wc.context_key_version IS auth_challenges.context_key_version AND wc.client_network_hmac IS auth_challenges.client_network_hmac AND wc.user_agent_hash IS auth_challenges.user_agent_hash AND wc.finished_at_ms IS NOT NULL)))",
    )
    .bind(access.now_ms)
    .bind(access.id.to_string())
    .bind(token_key_version)
...
async fn refresh_access_postgres(
    ...
    // concurrent method-specific terminal transaction either observes this refresh before
    // advancing replay/counter state, or commits its handoff before the following UPDATE
    // statements take fresh snapshots.
    ...
    .await?;
    sqlx::query(
        "UPDATE auth_challenges SET status=CASE WHEN attempts_used=max_attempts THEN 'exhausted' ELSE 'pending' END,attempt_claim_id=NULL,attempted_method=NULL,attempt_started_at_ms=NULL,attempt_expires_at_ms=NULL,revision=revision+1,updated_at_ms=$1 WHERE id=$2 AND token_key_version=$3 AND token_hmac=$4 AND context_key_version IS NOT DISTINCT FROM $5 AND client_network_hmac IS NOT DISTINCT FROM $6 AND user_agent_hash IS NOT DISTINCT FROM $7 AND status='verification_pending' AND created_at_ms<=$8 AND expires_at_ms>$9 AND attempt_expires_at_ms<=$10 AND NOT ((attempted_method='totp' AND EXISTS(SELECT 1 FROM totp_verified_handoffs h WHERE h.challenge_id=auth_challenges.id AND h.attempt_claim_id=auth_challenges.attempt_claim_id AND h.challenge_revision=auth_challenges.revision AND h.reserved_at_ms=auth_challenges.attempt_started_at_ms AND h.verification_expires_at_ms=auth_challenges.attempt_expires_at_ms)) OR (attempted_method='webauthn' AND EXISTS(SELECT 1 FROM webauthn_ceremonies AS wc WHERE wc.kind='authentication' AND wc.status IN ('consumed','rejected') AND wc.auth_challenge_id=auth_challenges.id AND wc.claim_id=auth_challenges.attempt_claim_id AND wc.user_id=auth_challenges.user_id AND wc.session_id IS NOT DISTINCT FROM auth_challenges.session_id AND wc.purpose=auth_challenges.purpose AND wc.auth_revision=auth_challenges.auth_revision AND wc.reserved_at_ms=auth_challenges.attempt_started_at_ms AND wc.verification_expires_at_ms=auth_challenges.attempt_expires_at_ms AND wc.context_key_version IS NOT DISTINCT FROM auth_challenges.context_key_version AND wc.client_network_hmac IS NOT DISTINCT FROM auth_challenges.client_network_hmac AND wc.user_agent_hash IS NOT DISTINCT FROM auth_challenges.user_agent_hash AND wc.finished_at_ms IS NOT NULL)))",
    )
    .bind(access.now_ms)
    .bind(access.id.into_uuid())
    .bind(token_key_version)
...
async fn refresh_user_purpose_sqlite(
    ...
    .await?;
    sqlx::query(
        "UPDATE auth_challenges SET status=CASE WHEN attempts_used=max_attempts THEN 'exhausted' ELSE 'pending' END,attempt_claim_id=NULL,attempted_method=NULL,attempt_started_at_ms=NULL,attempt_expires_at_ms=NULL,revision=revision+1,updated_at_ms=? WHERE user_id=? AND purpose=? AND status='verification_pending' AND created_at_ms<=? AND expires_at_ms>? AND attempt_expires_at_ms<=? AND NOT ((attempted_method='totp' AND EXISTS(SELECT 1 FROM totp_verified_handoffs h WHERE h.challenge_id=auth_challenges.id AND h.attempt_claim_id=auth_challenges.attempt_claim_id AND h.challenge_revision=auth_challenges.revision AND h.reserved_at_ms=auth_challenges.attempt_started_at_ms AND h.verification_expires_at_ms=auth_challenges.attempt_expires_at_ms)) OR (attempted_method='webauthn' AND EXISTS(SELECT 1 FROM webauthn_ceremonies AS wc WHERE wc.kind='authentication' AND wc.status IN ('consumed','rejected') AND wc.auth_challenge_id=auth_challenges.id AND wc.claim_id=auth_challenges.attempt_claim_id AND wc.user_id=auth_challenges.user_id AND wc.session_id IS auth_challenges.session_id AND wc.purpose=auth_challenges.purpose AND wc.auth_revision=auth_challenges.auth_revision AND wc.reserved_at_ms=auth_challenges.attempt_started_at_ms AND wc.verification_expires_at_ms=auth_challenges.attempt_expires_at_ms AND wc.context_key_version IS auth_challenges.context_key_version AND wc.client_network_hmac IS auth_challenges.client_network_hmac AND wc.user_agent_hash IS auth_challenges.user_agent_hash AND wc.finished_at_ms IS NOT NULL)))",
    )
    .bind(now_ms)
    .bind(user_id.to_string())
    .bind(purpose.as_str())
...
async fn refresh_user_purpose_postgres(
    ...
    // This serializes bulk expiry recovery with a method-specific transaction that locks one of
    // these rows before committing its durable terminal handoff in another table.
    sqlx::query(
    ...
    .await?;
    sqlx::query(
        "UPDATE auth_challenges SET status=CASE WHEN attempts_used=max_attempts THEN 'exhausted' ELSE 'pending' END,attempt_claim_id=NULL,attempted_method=NULL,attempt_started_at_ms=NULL,attempt_expires_at_ms=NULL,revision=revision+1,updated_at_ms=$1 WHERE user_id=$2 AND purpose=$3 AND status='verification_pending' AND created_at_ms<=$4 AND expires_at_ms>$5 AND attempt_expires_at_ms<=$6 AND NOT ((attempted_method='totp' AND EXISTS(SELECT 1 FROM totp_verified_handoffs h WHERE h.challenge_id=auth_challenges.id AND h.attempt_claim_id=auth_challenges.attempt_claim_id AND h.challenge_revision=auth_challenges.revision AND h.reserved_at_ms=auth_challenges.attempt_started_at_ms AND h.verification_expires_at_ms=auth_challenges.attempt_expires_at_ms)) OR (attempted_method='webauthn' AND EXISTS(SELECT 1 FROM webauthn_ceremonies AS wc WHERE wc.kind='authentication' AND wc.status IN ('consumed','rejected') AND wc.auth_challenge_id=auth_challenges.id AND wc.claim_id=auth_challenges.attempt_claim_id AND wc.user_id=auth_challenges.user_id AND wc.session_id IS NOT DISTINCT FROM auth_challenges.session_id AND wc.purpose=auth_challenges.purpose AND wc.auth_revision=auth_challenges.auth_revision AND wc.reserved_at_ms=auth_challenges.attempt_started_at_ms AND wc.verification_expires_at_ms=auth_challenges.attempt_expires_at_ms AND wc.context_key_version IS NOT DISTINCT FROM auth_challenges.context_key_version AND wc.client_network_hmac IS NOT DISTINCT FROM auth_challenges.client_network_hmac AND wc.user_agent_hash IS NOT DISTINCT FROM auth_challenges.user_agent_hash AND wc.finished_at_ms IS NOT NULL)))",
    )
    .bind(now_ms)
    .bind(user_id.into_uuid())
    .bind(purpose.as_str())
    ...
    pub async fn record_auth_challenge_failure(
        ...
        let rows_affected = match self {
            Self::Sqlite(pool) => {
                ...
                let rows = sqlx::query(
                    "UPDATE auth_challenges SET status=CASE WHEN attempts_used=max_attempts THEN 'exhausted' ELSE 'pending' END,attempt_claim_id=NULL,attempted_method=NULL,attempt_started_at_ms=NULL,attempt_expires_at_ms=NULL,revision=revision+1,updated_at_ms=? WHERE id=? AND token_key_version=? AND token_hmac=? AND context_key_version IS ? AND client_network_hmac IS ? AND user_agent_hash IS ? AND revision=? AND status='verification_pending' AND attempt_claim_id=? AND attempted_method=? AND (attempt_expires_at_ms>? OR (attempted_method='webauthn' AND EXISTS(SELECT 1 FROM webauthn_ceremonies AS wc WHERE wc.kind='authentication' AND wc.status='rejected' AND wc.auth_challenge_id=auth_challenges.id AND wc.claim_id=auth_challenges.attempt_claim_id AND wc.user_id=auth_challenges.user_id AND wc.session_id IS auth_challenges.session_id AND wc.purpose=auth_challenges.purpose AND wc.auth_revision=auth_challenges.auth_revision AND wc.reserved_at_ms=auth_challenges.attempt_started_at_ms AND wc.verification_expires_at_ms=auth_challenges.attempt_expires_at_ms AND wc.context_key_version IS auth_challenges.context_key_version AND wc.client_network_hmac IS auth_challenges.client_network_hmac AND wc.user_agent_hash IS auth_challenges.user_agent_hash AND wc.finished_at_ms IS NOT NULL))) AND expires_at_ms>? AND EXISTS(SELECT 1 FROM users AS u JOIN user_auth_state AS uas ON uas.user_id=u.id WHERE u.id=auth_challenges.user_id AND u.status='active' AND u.deleted_at_ms IS NULL AND uas.auth_revision=auth_challenges.auth_revision) AND (session_id IS NULL OR EXISTS(SELECT 1 FROM auth_sessions AS s WHERE s.id=auth_challenges.session_id AND s.user_id=auth_challenges.user_id AND s.status='active' AND s.auth_revision=auth_challenges.auth_revision AND s.last_seen_at_ms<=? AND s.idle_expires_at_ms>? AND s.absolute_expires_at_ms>?))",
                )
